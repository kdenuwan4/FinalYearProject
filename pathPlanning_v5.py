...
Module : Central Intelligence & Mapping + LED Communication Layer
Author : Amunugama H.M.K.D. (E/21/029)
Version: 5.0 (LED Communication & Navigation Signalling)
Date   : May 23, 2026

Description:
Upgrades the v4 active navigation execution system with a modular LED
communication layer. NavigationCommand outputs are translated into
deterministic 2x2 binary LED panel patterns displayed on a WALL-MOUNTED
panel. Robots read this panel using their onboard cameras and decode the
pattern to determine their next action.

Communication Concept:
  Central controller computes NavigationCommand per robot.
  Command is translated to a unique 4-bit LED pattern (A/B/C/D).
  Wall panel displays the pattern physically.
  Robot's onboard camera reads the 2x2 pattern and decodes the command.
  Robot executes accordingly (turn, move, stop).

  [ A ][ B ]      <- top row
  [ C ][ D ]      <- bottom row

  All 7 commands have unique patterns — no two commands share a pattern.
...
Architecture Layers:
  ...
  COMMUNICATION  - Commands mapped to unique 2x2 LED binary patterns.
                   Wall panel simulation window for visual debugging.
  VISUALISATION  - Engineering-style overlay with LED panel state per robot.

Fixes from submitted v5 draft:
  [Fix]  STOP and IDLE shared pattern (0,0,0,0) — now unique.
  [Fix]  LED panel architecture was per-robot — corrected to wall-mounted
         shared panel. One physical panel, multiplexed by robot ID prefix.
  [Fix]  Panel removed on robot disappear — wall panel persists.
  [Fix]  LED display in info panel was plain text — now rendered as a
         visual 2x2 grid with coloured circles (ON=green, OFF=dark).
  [Add]  Dedicated "LED Panel" simulation window showing the actual pattern
         as large coloured circles — what the robot camera would see.
  [Add]  Robot ID shown on panel window so multi-robot is traceable.
  [Add]  Pattern lookup table printed at startup for reference.

Preserved from v4 exactly:
...
#
#  The physical panel is WALL-MOUNTED and shared.
#  In a multi-robot system it is time-multiplexed: each robot ID
#  gets a dedicated display slot or the panel switches between robots.
#  For single-robot use it simply shows the current command pattern.
#
#  Hardware write is suppressed if the pattern has not changed
#  (avoids redundant updates to the physical panel driver).
# ================================================================
class LEDPanelController:
    """
    Manages LED panel state for one robot channel.
    Translates NavigationCommand -> 4-bit pattern -> hardware write.
    """
    ...
    def set_command(self, command: NavigationCommand):
        ...
        self._hardware_write(pattern, command)

    def _hardware_write(self, pattern: Tuple[int,int,int,int],
                        command: NavigationCommand):
        """
        Simulated hardware transmission.
        Replace this method body with actual GPIO / serial / VLC driver call.
        """
        a, b, c, d = pattern
        print(
            f"[LED PANEL] Robot {self.panel_state.robot_id:2d} "
            f"| {command.value:<14s} "
            f"| [{a}][{b}]  (A B)"
            f"\n{'':>36}  | [{c}][{d}]  (C D)"
        )
...
class LEDCommunicationController:
    ...
    Wall panel architecture note:
      ...
      When a robot disappears from camera, its panel channel retains
      the last command until the robot is re-detected or a STOP is issued.
...
def main():
    ...
    print("\n=== Navigation + LED Communication System v5 ===")
