...
Module : Central Intelligence & Mapping + Static A* Path Planning
Author : Amunugama H.M.K.D. (E/21/029)
Version: 1.0 (Static A* Integration)
Date   : May 13, 2026

Description:
This version upgrades the perception pipeline by integrating static A*
path planning on top of the existing vision system.

System capabilities:
- Continuous arena boundary tracking
- Perspective transformation (top-down warped grid)
- ArUco-based robot localization
- Robot orientation estimation
- Static A* shortest-path planning
- Path visualization on warped grid
- Graceful fallback when boundary is temporarily lost

Controls:
Q / ESC  -> Quit
L        -> Toggle manual boundary lock
D        -> Toggle edge debug window
=============================================================================
"""
...
def draw_obstacles(img, grid, rows, cols):
    """Fill obstacle cells with a dark rectangle and an X."""
    h, w   = img.shape[:2]
    cell_w = w // cols
    cell_h = h // rows
    for r in range(rows):
...
def draw_path(img, path, rows, cols):
    """Draw magenta path line and cyan goal circle."""
    if not path:
        return
    h, w   = img.shape[:2]
    cell_w = w // cols
...
def draw_grid(img, rows, cols, color=(0, 255, 0), thickness=2):
    """Grid lines and cell labels — drawn last so they sit on top."""
    h, w   = img.shape[:2]
    cell_w = w // cols
    cell_h = h // rows
...
def project_grid_back(frame, ordered, rows, cols, warp_w, warp_h, M_inv):
    """Re-project flat grid lines onto the angled camera view."""
    cell_w = warp_w // cols
    cell_h = warp_h // rows
...
def main():
    ...
    last_M       = None
    last_M_inv   = None
    last_warped  = None   # last good clean warp (no drawings)

    manual_lock = False
    show_debug  = False
    prev_time   = time.time()

    print("\n=== Static A* Arena Tracker ===")
    print("  Q / ESC -> quit")
    ...
    while True:
        ...
        if last_ordered is not None:
            ...
            if rect_found:
                # Fresh warp this frame — save as clean base
                last_warped = cv2.warpPerspective(frame, last_M, (WARP_W, WARP_H))

            # Work on a drawing copy so last_warped stays clean for next frame
            ...
            if rect_found:
                ...
                if ids is not None:
                    for i in range(len(ids)):
                        ...
                        # Draw order matters:
                        # obstacles → path → highlight → robot dot & arrow → labels
                        # grid lines drawn last (after loop) so they sit on top
                    ...
                    # No robots — still show obstacles on the grid
                    draw_obstacles(warped_draw, occupancy_grid, GRID_ROWS, GRID_COLS)

                # Grid lines always drawn last so they appear on top of everything
